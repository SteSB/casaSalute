...
class UtenteRepository(BaseRepository):
    ...
    def createUtente(self):     # POST
        ...
        existing_user = UtenteModel.query.filter_by(email=args['email']).first()
        if existing_user:
    ...
    def login(self):
        try:
            ...
            utente = UtenteModel.query.filter_by(email=email, password=password).first()
            if utente is None:
            ...
            if MedicoModel.query.filter_by(id_utente=utente.id_utente).first():
                ...
                role = "segretaria"
            return {"token": token, "utente": str(utente.id_utente), "ruolo": role}, 200    #, "utente": utente.to_dict()
